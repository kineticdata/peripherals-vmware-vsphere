...
class VmwareVsphereVirtualmachineResetV1
  ...
  # virtual machine that is to be reset.  Once located, it sends a signal 
  # to reset the VM.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  ...
  def execute()
    ...
    begin
      # Attempt to reset the virtual machine
      ...
      if exception.cause.is_a? com.vmware.vim25.InvalidPowerState
        raise "Unable to reset a Virtual Machine that is currently in a suspended state."
      else
        raise
      end
    end
