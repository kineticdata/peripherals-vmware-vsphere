...
class VmwareVsphereVirtualmachinePoweronV1
  ...
  # virtual machine that is to be powered on.  Once located, it sends a signal 
  # to power the VM on and waits for the VM to notify it that it is starting the
  # boot sequence.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  ...
  def execute()
    ...
    begin
      # Attempt to power on the virtual machine
      task = vm.powerOnVM_Task(nil)
      ...
      if exception.cause.is_a? com.vmware.vim25.InvalidPowerState
        raise "Unable to power on a Virtual Machine that is already running."
      else
        raise
      end
    end
