...
class VmwareVsphereVirtualmachineShutdownV1
  ...
  # virtual machine that is to be shut down.  Once located, it sends a signal 
  # to shut down the VM and waits for the VM to notify it that it is starting the
  # shut down sequence.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  ...
  def execute()
    ...
    begin
      ...
      if exception.cause.is_a?(com.vmware.vim25.InvalidPowerState)
        raise "Unable to shut down a virtual machine that is not powered on."
      # If VMWare tools is not installed on the host
      elsif exception.cause.is_a?(com.vmware.vim25.ToolsUnavailable)
        raise "Unable to shut down a virtual machine that does not have the VMWare Tools package installed."
